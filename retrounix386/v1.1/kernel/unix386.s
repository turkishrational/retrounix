; ****************************************************************************
; UNIX386.ASM (RETRO UNIX 386 Kernel) - v0.2.1.6
; ----------------------------------------------------------------------------
...
;
; [ Last Modification: 24/07/2022 (v0.2.1.6) ]
;	 04/02/2016 - Retro UNIX 386 v1.1 (Retro UNIX Kernel v0.2.1.0)	
;	 31/12/2021 - BugFix and Code Optimization (v0.2.1.1)
;	 03/03/2022 - Code Optimization and BugFix (v0.2.1.2) - 02/01/2022
;	 04/04/2022 - Inode Table/List Address Modification (v0.2.1.2) -u5.s-
;	 22/04/2022 - BugFix ('mget' -u5.s-)
;	 29/04/2022 - BugFix ('syschmod' -u2.s-) (v0.2.1.2)
;	 ; (v0.2.1.3)
;	 09/05/2022 - u2.s (sysstat), u7.s (sysumount), diskinit.inc  
;	 15/05/2022 - u2.s (namei), u7.s (sysumount)
;	 01/06/2022 - BugFix ('set_date_time' -u0.s-) (v0.2.1.3)
;	 ; (v0.2.1.4)
;	 13/06/2022 - LPT1 (/dev/lpr) printing modification (v0.2.1.4)
;	 14/06/2022 - BugFix (video.inc,'set_cpos' <- u9.s 'putc' carry flg)
;	 ; (v0.2.1.5)
;	 08/07/2022 - diskio.s (diskio simplified by removing the unused code) 
;	 ; (v0.2.1.6) 
;	 15/07/2022 - Simplified and Improved Buffer Handling -u8.s-
;	 16/07/2022 - Code Optimization
;	 17/07/2022 - Code Optimization
;	 18/07/2022 - Code Optimization
;	 19/07/2022 - BugFix (File Offset after disk write error, u6.s, u8.s)
;	 24/07/2022 - keyboard.s (simplified & optimized 'getc_int')	
;
...
;
; ****************************************************************************
; 31/12/2021 - 15/07/2022
; nasm unix386.s -l unix386.txt -o unix386 -Z error.txt
; ('unixcopy' utility is used for updating retro unix kernel on runix fs disk)

; 02/01/2022 - Code Optimization
; Runix Kernel v0.2.1.2
; 24/12/2021 - BugFix and Code Optimization
; Runix Kernel v0.2.1.1 
...
; 02/01/2022 (Retro UNIX 386 v1.1, Runix Kernel v0.2.1.2)
;; 27/12/2013
...
msg_out_of_memory:
	...
	db	0Dh, 0Ah
_int13h_48h_buffer: ; 02/01/2022 (Runix Kernel v0.2.1.2, 'diskinit.inc')
...
%include 'diskinit.inc' ; 07/03/2015
	...
	; 02/01/2022
	%define KERNELFSIZE KEND-KSTART 
	;
 	; 20/08/2014
	...
	;
	;;mov	cx, (KEND - KLOAD)/4
	;mov	cx, (KERNELFSIZE+3)/4 ; 02/01/2022
	mov	cx, (KERNELFSIZE+1)/2 ; 02/01/2022
	xor	si, si
	xor	di, di
	;rep	movsd
	rep	movsw ; 02/01/2022
	;
	push	es ; 0
	push	L17
	retf
...
_hang:	
	...
	mov	ah, 0Eh		; Yellow character 
				; on black background
	; 24/12/2021 (32 bit reg push-pop)
	; 07/09/2014
nxtl:
	push	ebx
	;
...
; 26/02/2022 - Real time clock (digital) output demo (sys emt)
x_rtci:
	dd	rtc_p	; (temporary demo code)	; 26/02/2022
...
msgKVER:
	;;;;db "Retro UNIX 386 v1.1 - Kernel v0.2.1.2 [29/04/2022]", 0
	;;;db "Retro UNIX 386 v1.1 - Kernel v0.2.1.3 [02/06/2022]", 0
	;;db "Retro UNIX 386 v1.1 - Kernel v0.2.1.4 [14/06/2022]", 0
	;db "Retro UNIX 386 v1.1 - Kernel v0.2.1.5 [12/07/2022]", 0
	db "Retro UNIX 386 v1.1 - Kernel v0.2.1.6 [24/07/2022]", 0
...
mem_1m_1k:   dw 0  ; Number of contiguous KB between
		   ;   1 and 16 MB, max. 3C00h = 15 MB.
	     dw 0  ; 02/01/2022 (Retro UNIX 386 v1.2)
