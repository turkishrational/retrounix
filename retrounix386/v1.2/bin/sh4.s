; ****************************************************************************
; sh386.s (sh4.s) - Retro Unix 386 v1.2 Shell - /bin/sh
; ----------------------------------------------------------------------------
;
...
; ((nasm sh4.s -l sh4.txt -o sh4 -Z error.txt))
...
; 14/06/2022 - Retro UNIX 386 v1.2
struc stat
...
newproc:
	...
	; Character devices must not be input/source
	test	al, 40h ; block device ?
	jz	short np1  ; no, character device !
	;
	; (disk blocks may be used as input source)
...
np11:
	...
	jc	short np3
	;
	mov	al, byte [stbuf+stat.mode+1]
	; Block devices and dirs must not be target
	test	al, 40h ; block device or directory ?
	jnz	short np4 ; yes, error

	test	al, 80h ; regular file ?
	jz	short np12 ; no, character device
...
np3: ;4:
	...
	; 13/06/2022 - Retro UNIX 386 1.2 (runix v2 inode)
	sys	_creat, esi, 01B6h  ; Decimal 438 = Octal 666 
		; mov r2,0f
...
stbuf:	; 14/06/2022 - Retro UNIX 386 v1.2
	resb 66	; sysstat output buffer
