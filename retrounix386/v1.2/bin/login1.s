; ****************************************************************************
; login386.s (login1.s) - Retro Unix 386 v1.2 - /bin/login - enter new user
; ----------------------------------------------------------------------------
...
START_CODE:
	...
	; 24/01/2022 (bss section is cleaned by the kernel)
...
lg18:
	...
	; (Retro UNIX 386 v2 file system inode)
	mov	eax, [pbuf+10] ; file size
	and	eax, eax
	jz	short lg19
	mov	esi, msgMail
	call 	mesg
...
;
ttyn:
	; 24/01/2022 - Retro UNIX 386 v1.2
	push	edi
	push	esi
	push	edx
	mov	byte [ttyname], 'x'
	sys	_fstat, 1, buf ; get tty file status
			       ...
			       ; (standard output) 
	jc	short er1
	sys	_open, dev, 0	; open '/dev' file (dir)
	jc	short er1
	;
	mov	si, [buf]	; inode number	
	mov	edi, eax	; file descriptor	
t1:
	; 24/01/2022
	sys	_read, edi, buf, 16 ; retro unix 386 v1.1
				    ; retro unix 386 v1.2	
	;sys	_read, edi, buf, 10 ; unix v1 dir entry !
				    ; retro unix 8086/386 v1
	jc	short er
	cmp	eax, 16		; retro unix 386 v1.1 & v1.2
	;cmp	eax, 10		; retro unix 8086/386 v1
	jne	short er
	...
	;mov	dx, [buf+2]	; check (14 byte) file name
	...
	cmp	dh, '9'		; > 'tty9'
	ja	short er	; yes	
	cmp	byte [buf+6], 0 ; remain bytes of the name is 0 ?
	jne	short er	; no (it is not tty0 .. tty9)
	mov	byte [ttyname], dh ; save tty name (last) char	
er:
	sys	_close, edi	; close '/dev' file (dir)
er1:
	;movzx	eax, byte [ttyname] ; return (last) tty name char
	; 24/01/2022
	; eax <= 16
	mov	al, [ttyname] ; '0' to '9' or 'x'
	pop	edx
	pop	esi
	pop	edi
	retn
...
;buf:	resb 34	  ; Retro UNIX 8086/386 v1 (32 byte inode, 32+2)
; 24/01/2022
buf:	resb 66   ; Retro UNIX 386 v1.2 (64 byte inode, 64+2)
ttyname: resw 1
