; ****************************************************************************
; init386.s (init8.s) - Retro Unix 386 v1.2 - /etc/init - sys initialization 
; ----------------------------------------------------------------------------
...
; ****************************************************************************
; init4.s (17/11/2015) - Retro UNIX 386 v1 & v1.1
;;init5.s (24/01/2022) - Retro UNIX 386 v1.2
; init6.s (24/01/2022-02/03/2022) - Retro UNIX 386 v1 & v1.1
; init7.s (09/02/2022-19/03/2022) - Retro UNIX 386 v1.2 & v1.1 (& v1.0)
;;init8.s (14/02/2022-20/03/2022) - Retro UNIX 386 v1.2
; init9.s (20/03/2022) - Retro UNIX 386 v1 & v1.1 	
;
...
; 02/03/2022
; 25/02/2022
; 14/02/2022 (init8.s) - Retro UNIX 386 v1.2
; 09/02/2022 (init7.s) - Retro UNIX 386 v1.2
; 24/01/2022 (init6.s) - Retro UNIX 386 v1 & v1.1
; 24/01/2022 (init5.s) - Retro UNIX 386 v1 & v1.1
; 17/11/2015 (init4.s)
; 23/10/2015 (init3.s)
; 14/10/2015 (init2.s)
; 17/09/2015
...
multiuser:
	...
	mov	edi, itabs
	xor	ebx, ebx
	mov	ecx, 0901h ; return tty8 serial port status
	sys	_gtty
	jc	short multiuser_e ; device not responding !
	mov	byte [edi], '8'
	add	edi, 4
multiuser_e:
	sub	ebx, ebx
	mov	ecx, 0A01h ; return tty9 serial port status
	sys	_gtty
	jc	short multiuser_s ; device not responding !
	mov	byte [edi], '9'	
	jmp	short multiuser_t

multiuser_s:
	cmp	byte [itabs], 0
	ja	short  multiuser_t
	
multiuser_x:	; 25/02/2022
	;mov	byte [itabs], 0 ; (disable serial port login)

	; 20/03/2022
	; (print serial port login disabled message)
	sys	_write, 1, td_msg, td_msg_size

;multiuser:
multiuser_t:	; 20/03/2022
	sys	_close, 0 ; close input file/tty
	;jc	short error
        sys	_close, 1 ; close output file/tty
	;jc	short error
...
i2:
	...
	; 02/03/2022
	;mov	ecx, eax ; 18/02/2022
	mov	edi, esi	
	call	dfork        ; go to make new init for this ttyx
	;mov	eax, ecx ; 18/02/2022
...
pwait:
	...
	; 02/03/2022
	jnc	short i8    ; (eax = process ID of the child)
...
o0:	
	...
	;; temporary test code - 20/03/2022
	;jnc	short o1 ; temporary - 20/03/2022
	;add	cl, '0'
	;mov	[termno], cl
	;sys	_msg, tempor_msg, 255, 0Fh
	;
	;jmp	short o4
o1:
	; 16/11/2015	
...
wtmprec:
	...
	cmp	byte [wtmp_err], 0
	ja	short i6 ; 18/02/2022

	sys	_open, wtmp, 1 ; open accounting file
	jc	short i7 ; 18/02/2022
	; 24/01/2022
	;jc	short i6
