; Retro UNIX 386 v1 Kernel (v0.2.0.22) - SYS1.INC
...
syswrite: ; < write to file >
	...
	;jc	error ; 13/05/2015, ax < 1
		; jsr r0,rw1 / get i-number in r1 of file to write
	; 01/02/2022
	;jnc	short syswrite_1
;rw_error:
	;jmp	error
	; 27/02/2022
	jc	short short syswrite_err
syswrite_1:
        test	ah, 80h
...
sysopen_5: ; 1: / r2 has index to u.fp list; r3, has index to fsp table
	...
	xor	eax, eax
	mov	[ebx+fsp+4], eax ; offset pointer (0)
	mov	[ebx+fsp+8], ax ; open count (0), deleted flag (0)

	mov	ax, [cdev]
	mov	[ebx+fsp+2], ax ; device number
...
syscreat: ; < create file >
	...
	; 12/02/2022 (Retro UNIX 386 v1.0, Kernel v0.2.0.18, 2022)
	; 27/12/2015 (Retro UNIX 386 v1.1)
	cmp	ax, 41 ; device inode ?
        ;jb	syscreat_0 ; yes
	; 02/02/2022
	jnb	short syscreat_1
	jmp	syscreat_0
syscreat_1:
	neg 	ax
	...
	pop	ecx ; mode
	; 12/02/2022
	jmp	short syscreat_3
        ;jmp	sysopen_1
        	; br op0
syscreat_2: ; 2: / file doesn't exist
	; 01/02/2022
	pop	eax ; mode
	;pop	ax
...
sysstty_8:
	...
	jmp	sysstty_pd_err ; 'permission denied !'
sysstty_21:
	; 29/10/2015
	xchg 	ch, cl
...
;sysgtty_5:
	...
	test	al, 1
	;jz	sysret
	jz	short sysgtty_10 ; 22/02/2022
