; Retro UNIX 386 v1 Kernel (v0.2.0.19) - SYS2.INC
...
sysexec:
	...
	; 03/02/2022 (Retro UNIX 386 v1.0, Kernel v0.2.0.18)
	; 23/10/2015
	; 19/10/2015
...
sysexec_17: ; 20/07/2015
	; 03/02/2022 - Retro UNIX 386 v1 (Kernel v0.2.0.18)
...
get_argp5:
	...
	call	get_physical_addr ; get physical address
        jc	short get_argp_err ; 03/02/2022 (short jump)
...
sysfstat:
	; 09/05/2022 (Retro UNIX 386 v1, Kernel v0.2.0.19)
	;	([idev] return in eax)
	;	0 = root device
	;	1 = mounted device (>0)
	; 03/02/2022
...
namei:
	; 15/05/2022
	; 14/05/2022 (mounted directory path, '..' method)
	; 03/02/2022
	; 18/10/2015 (nbase, ncount)
	; 12/10/2015
	; 21/08/2015
...
namei_1: ; 1:
	test	byte [esi], 0FFh
namei_10: ; 03/02/2022 (jump from namei_8)
	jz	short getf4
	;jz	nig
...
namei_8: ; 3
	...
	;mov	bx, [u.dirbuf]
		; mov u.dirbuf,r1 / move i-node number in directory 
				; / entry to r1
	;;;;
	; 14/05/2022 - Retro UNIX (8086/386) feature only !
...
maknod1: ; 1: / scan for a free i-node (next 4 instructions)
	...
	; 12/02/2022 - Retro UNIX 386 v1.0 (Kernel v0.2.0.18)
	; (inode count check)
