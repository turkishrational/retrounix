; ****************************************************************************
; pr5.s (pr8086.s) - print file - by Erdogan Tan - 25/06/2022
; ----------------------------------------------------------------------------
; Retro UNIX 8086 v1 - file print utility 
;
...
; ((nasm pr5.s -l pr5.txt -o pr5 -Z error.txt))

; pr0.s (29/06/2022, Retro UNIX 8086 v1)
...
; pr5.s (10/07/2022, Retro UNIX 8086 v1) ; (LF -> CRLF)
...
[BITS 16] ; 16-bit intructions (for 8086 or x86 real mode)

[ORG 0] 

START_CODE:
	; 29/06/2022
	; 26/06/2022 (16 bit version, pr0.s)
	; 25/06/2022 (32 bit version, pr1.s)
...
prn_6:
	sys	_fstat, ax, stbuf
	; 26/06/2022
	mov	ax, [stbuf+stat.mtime]
	mov	dx, [stbuf+stat.mtime+2]
prn_7:
	mov	[mtime], ax
	mov	[mtime+2], dx
...
p_next_file:
	...
	;mov	[fbuf], ax ; 0	  ; open file number
	; 26/06/2022
	mov	[fbuf+2], ax ; 0 ; character count
...
prn_8:
	...
	mov	si, di ; file name address (on stack)
...
prn_10:
	mov	cx, di
	;sys	_write, [prnfnum], di, dx
	sys	_write
	jc	short prn_err

	mov	cx, PageHeader
	mov	dl, 6
	;sys	_write, [prnfnum], PageHeader, dx
	sys	_write
	jc	short prn_err

	mov	ax, [pgnumber]
	xor	dx, dx ; 29/06/2022
	call	write_pg_num
	jc	short prn_err	
...
getc:
	mov	ax, [fbuf+2] ; character count
	and	ax, ax
	jnz	short gch1
gch0:
	mov	bx, [fbuf] ; file descriptor/number
	mov	cx, fbuf + 6 ; read buffer address
	mov 	[fbuf+4], cx ; character offset
	sub	dx, dx
	mov	dl, 128
...
gch1:
	dec	ax
	mov	[fbuf+2], ax ; character count in buffer
	mov	bx, [fbuf+4] ; current character offset
	xor	ah, ah
	mov	al, [bx]
	inc	bx
	mov	[fbuf+4], bx ; next character offset
	retn 	
gch2:
	xor	ax, ax
gch3:
...
program_msg:
	db  0Dh, 0Ah
	db  'Retro UNIX 8086 v1 PRINT Utility by Erdogan TAN - 10/07/2022'
	db  0Dh, 0Ah, 0
usage_msg:
	db  0Dh, 0Ah
